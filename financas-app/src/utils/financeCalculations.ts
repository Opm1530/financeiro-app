import { Transaction, Goal, Contract } from '../types';
...
export const FinanceCalculator = {
    ...
    calculateRecurrence: (contracts: Contract[]) => {
        const today = new Date();
        today.setHours(0, 0, 0, 0); // Zera as horas para comparação precisa de datas

        // Filtra contratos recorrentes que estão ativos
        const activeRecurringContracts = contracts.filter(contract => {
            // 1. Deve ser do tipo RECURRING
            if (contract.targetType !== 'RECURRING') {
                return false;
            }

            // 2. Verifica se o contrato já iniciou
            const startDate = new Date(contract.startDate);
            startDate.setHours(0, 0, 0, 0);
            
            if (startDate > today) {
                return false; // Contrato ainda não começou
            }

            // 3. Verifica se o contrato ainda está vigente
            // Se não tem endDate, considera como ativo indefinidamente
            if (!contract.endDate) {
                return true;
            }
            ...
            if (endDate < today) {
                return false; // Contrato já expirou
            }

            return true; // Contrato está ativo
        });

        // Soma os valores dos contratos ativos
        const totalRecurrence = activeRecurringContracts.reduce((acc, contract) => {
            return acc + parseFloat(contract.value);
        }, 0);

        return totalRecurrence;
    },

    calculateActiveContracts: (contracts: Contract[]) => {
        const today = new Date();
        today.setHours(0, 0, 0, 0); // Zera as horas para comparação precisa de datas
        ...
        // Soma os valores dos contratos ativos
        const totalActiveContracts = activeRecurringContracts.length;
